...
cs_table = [
  {
    "category": "가격비교",
    "result": "답변 1 : 가격비교 매칭 방법은 'http://imgshopping.naver.net/admin/notice/g98cytj1o5y.png'을 참조해주세요. 답변 2 : 가격비교제외상품으로 나오는 경우 괄호 안에 가격비교 제외된 사유를 확인해주세요. 답변 3 : 가격비교제외상품의 가격비교 제외 사유가 '원부 내 포함대상이 아님'일 경우 다음의 내용을 확인해주세요. 상품 기본정보(대표 이미지, 제조사, 브랜드, 상품명, 상품코드) 불일치, 여러 상품을 취급하는 모음전 상품은 매칭이 불가합니다. 1. 상품명에 상품코드가 없음\n적용 카테고리 : 디지털/가전, 식품, 화장품/미용\n[예시] 상품코드 추가\n삼성전자 정품 27인지 모니터 (X)\n삼성전자 S27R35A 27인치 모니터 (O)\n2. 상품명에 '용량/수량' 정보가 없음\n[예시] 용량 추가\n센트룸 실버 프로 멀티비타민 (X)\n센트룸 실버 프로 멀티비타민 1367mg x 120정 (O)\n3. 모음전 상품은 가격비교 매칭 불가\n- '택1' 등 선택형 상품, 모든 묶음형식 상품\n- 상품 및 조건을 여러 개 표기한 상품\n상품명이 가격비교명(조건)과 일치해야 합니다. 답변 4 : 가격비교제외상품의 가격비교 제외 사유가 '원부 배송비 기준 위반'일 경우 다음의 내용을 확인해주세요. 수량 단위로 배송비가 발생하거나, 가격비교 배송비 조건을 초과한 배송비 상품은 가격비교에서 제외됩니다.\n\n1. '수량별 배송비' 발생\n무게/부피가 실제 크지 않은데 특정 수량 단위로 배송비를 부과하는 상품\n[예시]\n샤오미 보조배터리 - 최대구매 수량 1개\n황사 마스크 - 10개당 배송비 부과\n→ 실제 발생하는 배송비로 수정해야 합니다.\n\n2. 가격비교 배송비 조건\n부피가 작은 상품군: 3,000원\n부피가 큰 상품군: 5,000원\n택배 배송비 불가능한 대형 상품군: 30,000원 답변 5 : 가격비교제외상품의 가격비교 제외 사유가 '원부 설치비 기준 위반'일 경우 다음의 내용을 확인해주세요. 에어컨, 보일러, 온수기 등 설치가 필요한 상품은 상품명에 '기본설치비 포함' 및 상세페이지에 추가비용 정보를 표기하지 않으면 매칭이 불가합니다. 설치 상품군 서비스 기준은 'https://adcenter.shopping.naver.com/board/notice_detail.nhn?page=1&noticeType=&noticeSeq=274384&srchKey=cont&srchVal=%EC%84%A4%EC%B9%98%EB%B9%84'를 참조해주세요. 답변 6 : 가격비교제외상품의 가격비교 제외 사유가 '원부기준 위반_기타'일 경우 다음의 내용을 확인해주세요.가격비교 매칭 가이드에 맞지 않은 상품은 가격비교에 노출할 수 없습니다. 제외된 사유는 'https://adcenter.shopping.naver.com/board/notice_detail.nhn?page=1&noticeType=&noticeSeq=276064&srchKey=ttl&srchVal=%EB%A7%A4%EC%B9%AD'을 통해 확인 가능합니다. 답변 7: 가격비교제외상품의 가격비교 제외 사유가 '원부 비대상 상품군'일 경우 가격비교를 서비스하지 않는 상품군으로 매칭이 불가능합니다. 상품 타입 : 중고, 리퍼, 전시, 반품, 스크래치 판매 방식 : 대여, 도매, 할부, 예약판매, 구매대행 답변 8 : 11번가,쿠팡 상품의 가격비교 매칭 요청은 오픈마켓/종합몰을 통해 상품정보가 수신됩니다. 따라서, 해당 오픈마켓/종합몰 담당자에게 매칭 요청해주시기 바랍니다. 답변 9 : 가격비교 매칭을 요청했는데 노출이 안되는 경우 매칭상태를 확인해주세요.가격비교 매칭상태를 확인 하는 방법은 가격비교를 요청했던 페이지 상품현황 및 관리> 서비스 상품 리스트에서 상품의 상태 및 요청 처리 결과를 확인할 수 있습니다. 표의 오른쪽 열에서 '상태'를 확인해주세요. 'https://adcenter.shopping.naver.com/iframe//product/manage/service/list.nhn'를 참조해주세요. 답변 10 : 가격비교 매칭상태가 매칭완료인데 안나오는 경우 [가격비교 매칭완료] 처리 후, 쇼핑검색결과 반영까지 업데이트 시간이 소요됩니다. 최대 8시간 내에 검색결과에 반영(노출)됩니다. 답변 11 : 가격비교 매칭 상태가 [추천중] 또는 [반려] 로 나오는 경우 다음과 같은 상품은 매칭이 불가능합니다. 1. 다른 옵션이 포함된 모음 상품명 2. 1개 또는 몇 개 단위 배송비 부과 상품 3. 원부와 사이즈,용량,수량이 다른 상품 답변 12 : 네이버쇼핑 가격비교 매칭 기준은 네이버 자체적으로 가격비교 서비스의 제공 및 각 상품의 매칭 필요성 여부를 판단하여 처리하고 있습니다. 자세한 기준은 'https://help.pay.naver.com/faq/content.help?faqId=13388'를 확인해주세요."
  }
]
@app.get("/csTest")
